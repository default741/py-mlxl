...
class _Utils:
    ...
    def _roc_threshold(y_train: np.array, y_prob: np.array) -> float:
        ...
        roc = pd.DataFrame({'tf': pd.Series(
            tpr - (1 - fpr), index=index), 'threshold': pd.Series(threshold, index=index)})
    ...
    def _pr_threshold(y_train: np.array, y_prob: np.array) -> float:
        ...
        roc = pd.DataFrame({'tf': pd.Series(
            precision[:-1] - recall[:-1], index=index), 'threshold': pd.Series(threshold, index=index)})
        roc = roc.iloc[(roc['tf'] - 0).abs().argsort()[:1]]
...
class _Model_Report:
    ...
    def _get_model_params(params: dict):
        if params['model_name'] == 'lightgbm':
            if params['lgbm_max_depth'] == -1:
                ...
                params['lgbm_num_leaves'] = int(
                    2 ** params['lgbm_max_depth'] / 2)
    ...
    def _train_pipeline(X_train: pd.DataFrame, y_train: np.array, model: object, sample_method: object):
        ...
        if sample_method is not None:
            pipeline = imbpipeline(
                steps=[('sample', sample_method), ('model', model)])
        else:
    ...
    def _generate_predictions(pipeline: object, data_tuple: tuple):
        ...
        thresh_roc = _Utils._roc_threshold(
            y_train=y_train, y_prob=results['y_prob_train'][:, 1])
        thresh_pr = _Utils._pr_threshold(
            y_train=y_train, y_prob=results['y_prob_train'][:, 1])

        results['y_pred_opt'] = [
            1 if j > thresh_roc else 0 for j in results['y_prob_test'][:, 1]]
        results['y_pred_pr'] = [
            1 if j > thresh_pr else 0 for j in results['y_prob_test'][:, 1]]
    ...
    def _get_kpi_metrics(params: dict, cv_results: dict, y_test: np.array, results: dict, thresh_roc: float, thresh_pr: float):
        cv_result_data = [
            params['model_name'], params['dataset_name'], params.get(
                'sample_method', 'balanced_data (None)'), 0.5, 'CV Scores',
            round(np.mean(cv_results["test_accuracy"]), 4), round(np.mean(
                cv_results["test_precision"]), 4), round(np.mean(cv_results["test_recall"]), 4),
            round(np.mean(cv_results["test_f1"]), 4), round(
                np.mean(cv_results["test_roc_auc"]), 4)
        ]

        test_result_data = [
            params['model_name'], params['dataset_name'], params.get(
                'sample_method', 'balanced_data (None)'), 0.5, 'Test Data',
            round(accuracy_score(y_test, results['y_pred_test']), 4), round(
                precision_score(y_test, results['y_pred_test']), 4),
            round(recall_score(y_test, results['y_pred_test']), 4), round(
                f1_score(y_test, results['y_pred_test']), 4),
            round(roc_auc_score(y_test, results['y_prob_test'][:, 1]), 4)
        ]

        roc_result_data = [
            params['model_name'], params['dataset_name'], params.get(
                'sample_method', 'balanced_data (None)'), round(thresh_roc, 3), 'ROC Threshold',
            round(accuracy_score(y_test, results['y_pred_opt']), 4), round(
                precision_score(y_test, results['y_pred_opt']), 4),
            round(recall_score(y_test, results['y_pred_opt']), 4), round(
                f1_score(y_test, results['y_pred_opt']), 4),
            round(roc_auc_score(y_test, results['y_prob_test'][:, 1]), 4)
        ]

        pr_result_data = [
            params['model_name'], params['dataset_name'], params.get(
                'sample_method', 'balanced_data (None)'), round(thresh_pr, 3), 'PR Threshold',
            round(accuracy_score(y_test, results['y_pred_pr']), 4), round(
                precision_score(y_test, results['y_pred_pr']), 4),
            round(recall_score(y_test, results['y_pred_pr']), 4), round(
                f1_score(y_test, results['y_pred_pr']), 4),
            round(roc_auc_score(y_test, results['y_prob_test'][:, 1]), 4)
        ]

        metrics = [cv_result_data, test_result_data,
                   roc_result_data, pr_result_data]
    ...
    def _get_decile_data(y_prob: np.array, y_pred: np.array, y_test: np.array, num_deciles: int) -> pd.DataFrame:
        prob_defect = pd.DataFrame(
            {'probabilities': y_prob, 'actual_class': y_test, 'pred_class': y_pred})
        labels = [f'D{i}' for i in range(1, num_deciles + 1)]

        prob_defect['Decile'] = pd.qcut(
            prob_defect['probabilities'], num_deciles, labels=labels)
        prob_defect['Range'] = pd.qcut(
            prob_defect['probabilities'], num_deciles, retbins=True)[0]
        ...
        report['Actual Non Defective'] = report['Total Engines'] - \
            report['Actual Defective']
        report['Predicted Non Defective'] = report['Total Engines'] - \
            report['Predictive Defective']
        report['Defect Rate'] = report['Actual Defective'] / \
    ...
    def _main(best_trials: list, data_dict: dict, sample_schema: dict, num_deciles: int):
        ...
        for trial in best_trials:
            ...
            X_train, y_train, X_test, y_test = _Model_Report._get_train_test_data(
                feature_set)

            model_params = _Model_Report._get_model_params(params=trial.params)
            model = _Model_Report._get_classifier(
                model_name=trial.params['model_name'], params=model_params)

            pipeline, cv_results = _Model_Report._train_pipeline(
                X_train=X_train, y_train=y_train, model=model, sample_method=sample_method)

            data_tuple = (X_train, y_train, X_test, y_test)
            results, thresh_pr, thresh_roc = _Model_Report._generate_predictions(
                pipeline, data_tuple)
            ...
            for category in ['y_pred_test', 'y_pred_opt', 'y_pred_pr']:
                decile_report[category] = _Model_Report._get_decile_data(
                    y_prob=results['y_prob_test'][:,
                                                  1], y_pred=results[category], y_test=results['y_true_test'], num_deciles=num_deciles
                )
...
class _Hyper_Parameter_Tuning:
    ...
    def _objective(self, trial):
        ...
        feature_set_name = trial.suggest_categorical(
            'dataset_name', self._tuning_config['data_set'])
...
class Tuning:
    ...
    def generate_model_report(self, best_trials: list, data_dict: dict, num_deciles: int) -> None:
        report = _Model_Report._main(best_trials=best_trials, data_dict=data_dict,
                                     sample_schema=self._sample_schema, num_deciles=num_deciles)
    ...
    def save_results(self, optuna_study: object, report: list, save_path: str) -> None:
        ...
        """
        joblib.dump({'optuna_study': optuna_study,
                    'report': report}, save_path)

    def compile_tuning(
        self, file_paths: dict, tuning_parameter: str, balanced: bool, n_trials: int, num_deciles: int, save_path: str, **kwargs
    ) -> None:
        ...
        selected_features = _Read_Data_File._read_csv_type(
            file_path=file_paths['selected_feature'], params=kwargs).to_dict(orient='list')
        ...
        optuna_study = self.tune_models(baseline_results=baseline_results,
                                        data_dict=data_dict, tuning_parameter=tuning_parameter, n_trials=n_trials)
...
if __name__ == '__main__':
    ...
    config = {
        'file_paths': {
            'X_train': '../data/interim_data/feature_selected_data_v1.joblib',
            'y_train': '../data/interim_data/feature_selected_data_v1.joblib',
            'selected_feature': '../data/interim_data/feature_selected_data_v1.joblib',
