...
from utils_reg import _Model_Utility
...
class _Feature_Importance:
    ...
    """
    @staticmethod
    def _gen_local_feature_importance(cf_object: object, query_instances: pd.DataFrame, total_CFs: int, desired_range: list) -> list:
        ...
        Args:
            ...
            desired_range (list): range for arget variable.
        ...
        try:
            for i in range(len(query_instances)):
                importance = cf_object.local_feature_importance(query_instances[i:i+1], total_CFs=total_CFs, desired_range=desired_range)
    ...
    @staticmethod
    def _gen_global_feature_importance(cf_object: object, query_instances: pd.DataFrame, total_CFs: int, desired_range: list) -> dict:
        ...
        Args:
            ...
            query_instances (pd.DataFrame): DataFrame of query_instances
            total_CFs (int): No:of CF's to generate while calculating feature importance.
            desired_range (list): desired range for target variable.

        Returns:
            dictionary: global feature importance w.r.t all Query Instances given.
        """
        global_feature_impt = dict()
        try:
            global_feature_impt = cf_object.global_feature_importance(query_instances, total_CFs=total_CFs, desired_range=desired_range).summary_importance
        except Exception as e:
            print(e)
        return global_feature_impt


class CounterFactualRegression:
    ...
    def __init__(self, model_meta_data: dict, desired_range: list, total_cfs: int, num_query_instances: int, method: str = 'genetic') -> None:
        ...
        Args:
            ...
            desired_range (list): Range for target variable.
    ...
    def generate_cf_object(self) -> tuple:
        ...
        model_object = Model(model=self.model_meta_data['model_object'], backend='sklearn', model_type='regressor')
    ...
    def generate_cf_meta_obj(self,feature:str,cf_object:object, query_instance:pd.DataFrame, permitted_range:dict = None) -> tuple:
        ...
        if (permitted_range is not None) and (feature in permitted_range.keys()):
            cf_object_meta = cf_object.generate_counterfactuals(query_instance,self.total_cfs, None, self.desired_range, {feature : permitted_range[feature]},[feature])
        else:
            cf_object_meta = cf_object.generate_counterfactuals(query_instance,self.total_cfs,None, self.desired_range,None,[feature])
    ...
    def generate_cf_actual(self, cf_object: object ,query_instances: pd.DataFrame,features_to_vary: list , permitted_range: dict = None)-> tuple:
        """
        Driver function to generate counter factuals.
        ...
        for idx in range(self.num_query_instances):
            ...
            for feature in features_to_vary:
                if feature in self.model_meta_data['numerical_columns']:
                    ...
                    try:
                        cf_vals_ft, cf_df_instance_ft = func_timeout(1*60,CounterFactualRegression.generate_cf_meta_obj, args = (
                        ...
                        instance_dict[feature] = CounterFactualRegression.calculate_min_actual_difference(actual_ft_instance = actual_ft_instance, cf_vals_ft = cf_vals_ft)
                        instance_dict_std[feature] = CounterFactualRegression.calculate_min_standardized_difference(self, feature = feature, actual_ft_instance = actual_ft_instance, cf_vals_ft = cf_vals_ft)
    ...
    def generate_counter_factuals(self, features_to_vary: list = None, permitted_range: dict = None) -> tuple:
        ...
        cf_object, query_instances = CounterFactualRegression.generate_cf_object(self)
        if (features_to_vary is not None and permitted_range is not None) or (features_to_vary is None and permitted_range is not None):
            ...
            final_feature_magnitude_change, final_feature_magnitude_change_standard,cf_df = CounterFactualRegression.generate_cf_actual(self,cf_object=cf_object, query_instances = query_instances, features_to_vary = features_to_vary,permitted_range = permitted_range)
            ...
            final_feature_magnitude_change,final_feature_magnitude_change_standard,cf_df = CounterFactualRegression.generate_cf_actual(self, cf_object=cf_object,query_instances = query_instances,features_to_vary = features_to_vary)
        ### Generating Feature Importance.     
        local_feature_impt = _Feature_Importance._gen_local_feature_importance(cf_object, query_instances, self.total_cfs, self.desired_range)
        global_feature_impt = _Feature_Importance._gen_global_feature_importance(cf_object, query_instances, self.total_cfs, self.desired_range)
...
if __name__ == '__main__':
    features_to_vary = ['MedInc','AveRooms']
    permitted_range = {}
    ...
    cfc = CounterFactualRegression(model_meta_data=model_meta_data, desired_range=desired_range, total_cfs=total_cfs, num_query_instances=num_query_instances)
    ...
    cf_df.to_csv('Regression_CF.csv',encoding='utf-8')
    print("Done")